...
source "hcloud" "archlinux" {
  ...
  ssh_agent_auth = true
  server_name = "archlinux-kubernetess-${ local.snapshotbuildtime }"
  snapshot_name = "archlinux-kubernetes-${ local.snapshotbuildtime }"
  snapshot_labels = {
    "packer.io/version" = "${packer.version}",
    "packer.io/build.time" = "${ local.snapshotbuildtime }",
    "os-flavor" = "archlinux",
    "archlinux/iso.release" = "${local.arch-release}",
    "image_type" = "archlinux-kubernetes"
  }
}
...
build {
  ...
  provisioner "file" {
    source = "files/etc/systemd/network/10-flannel.link"
    destination = "/mnt/etc/systemd/network/10-flannel.link"
  }
  ...
  provisioner "file" {
    source = "files/etc/modules-load.d/00-k8s.conf"
    destination = "/mnt/etc/modules-load.d/00-k8s.conf"
  }

  provisioner "file" {
    source = "files/etc/sysctl.d/00-k8s.conf"
    destination = "/mnt/etc/sysctl.d/00-k8s.conf"
  }

  provisioner "shell" {
    inline = ["mkdir -p /mnt/etc/systemd/system/kubelet.service.d"]
  }

  provisioner "file" {
    source = "files/etc/systemd/system/kubelet.service.d/20-hcloud.conf"
    destination = "/mnt/etc/systemd/system/kubelet.service.d/20-hcloud.conf"
  }

  provisioner "file" {
    source = "files/etc/containers/storage.conf"
    destination = "/mnt/etc/containers/storage.conf"
  }

  provisioner "file" {
    source = "files/etc/crio/crio.conf.d/00-plugin-dir.conf"
    destination = "/mnt/etc/crio/crio.conf.d/00-plugin-dir.conf"
  }
